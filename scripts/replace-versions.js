...
if (isRollback) {
  console.log(
    `Reverting '@pagerduty/backstage-plugin-common' version in ${package.name}...`,
  );
  ...
  if (matches === null) {
    console.error(
      `\x1b[35mError: Expected '@pagerduty/backstage-plugin-common' to be a version, but got '${version}'\x1b[0m`,
    );
    exit(1);
  }
  ...
  console.log(
    `Replacing '@pagerduty/backstage-plugin-common' version in ${package.name}...`,
  );

  var workspaceProtocol =
    package.dependencies['@pagerduty/backstage-plugin-common'];
  ...
  if (matches === null) {
    ...
    if (matches !== null) {
      console.warn(
        `Expected '@pagerduty/backstage-plugin-common' already has a version. No need to replace it.`,
      );
      exit(0);
    }

    console.error(
      `\x1b[35mError: Expected '@pagerduty/backstage-plugin-common' to be a workspace protocol, but got '${workspaceProtocol}'\x1b[0m`,
    );
    exit(1);
  }
