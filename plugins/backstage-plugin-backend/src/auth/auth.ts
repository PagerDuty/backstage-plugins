...
export async function getAuthToken(accountId?: string): Promise<string> {
  ...
  const key =
    accountId && accountId !== ''
      ? accountId
      : authPersistence.defaultAccount ?? '';

  if (
    authPersistence.accountTokens[key]?.authToken !== '' &&
    ((await checkForOAuthToken(key)) ||
      authPersistence.accountTokens[key]?.authToken.includes('Token'))
  ) {
    ...
    if (!config.getOptional('pagerDuty.accounts')) {
      ...
      if (!config.getOptionalString('pagerDuty.apiToken')) {
        ...
        if (!config.getOptional('pagerDuty.oauth')) {
          ...
          !config.getOptionalString('pagerDuty.oauth.clientId') ||
          !config.getOptionalString('pagerDuty.oauth.clientSecret') ||
          !config.getOptionalString('pagerDuty.oauth.subDomain')
        ) {
          logger.error(
